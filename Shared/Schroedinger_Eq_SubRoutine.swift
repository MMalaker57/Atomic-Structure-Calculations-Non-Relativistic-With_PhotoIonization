...
class Schroedinger_Eq_SubRoutine: NSObject {
    ...
    func outward_integration(initial_r_list: [Double], initial_KE_list: [Double], number_blocks: Double, l_number: Double, trial_psi0_guess: Double, number_points: Double, KE_index_tuple: (KE_cross_index_block: Int, KE_cross_block_number: Int, KE_cross_index_univ: Int), outward_index_tuple: (outer_radius_index_block: Int, outer_radius_block_number: Int, outer_radius_index_univ: Int)){
        ...
        // variables to hold index values from KE tuple so values from tuple doesnt have to be called everytime
    ...
    /// Name: outward_integration_Photo
    /// Description: Uses numerov numerical integration method to solve schroedinger equation starting at origin (r=0.0)for certain neutral atom
    ///     This function ignores the inward/outward crossing point because photoionization functions cannot have the inward integration.
    ///
    /// - Parameters:
    ///   - initial_r_list: Input initial radius list
    ///   - initial_KE_list: Input Kinetic energy term list
    ...
    func outward_integration_Photo(initial_r_list: [Double], initial_KE_list: [Double], number_blocks: Double, l_number: Double, trial_psi0_guess: Double, number_points: Double, KE_index_tuple: (KE_cross_index_block: Int, KE_cross_block_number: Int, KE_cross_index_univ: Int), outward_index_tuple: (outer_radius_index_block: Int, outer_radius_block_number: Int, outer_radius_index_univ: Int)){
        ...
        let temp_wavefunction_list = schroed_functional_functions_inst.schrodinger_numerov_method(r_list: initial_r_list, gn_coeff_list: initial_KE_list, psi0: 0.0, psi1_guess: trial_psi0_guess)
        
        schroed_wavefunction_values_inst.outward_Pwavefunction_tuple_array.append((wavefunction_list: temp_wavefunction_list, block_number: (1.0)))
        
//        for i in stride(from: 0, to: initial_r_list.count, by: 1){
//            // Range of r list values from each block based on universal index
////            let temp_r_list: [Double] = initial_r_list
//            // Range of KE list values from each block based on universal index
////            let temp_gn_list: [Double] = initial_KE_list
//            // Calculated wavefunction values from r list and KE list above
//            let temp_wavefunction_list = schroed_functional_functions_inst.schrodinger_numerov_method(r_list: initial_r_list, gn_coeff_list: initial_KE_list, psi0: 0.0, psi1_guess: trial_psi0_guess)
//            
//            schroed_wavefunction_values_inst.outward_Pwavefunction_tuple_array.append((wavefunction_list: temp_wavefunction_list, block_number: (1.0)))
...
//        }
    ...
    //This is a copy of the subroutine above. This was done to prevent breaking that code when adding the photoionization part
    
    
    /// Name: schroedinger_photoionization_subroutine
    /// Description: Subroutine that combines all functions above to calculate self consistent wavefunction values for a given energy, potential input etc.
    ///     Designed to calculate the end state wavefunction post-ionization. The other routine should be used for the initial state
    ///     Explicitely for use with POSITIVE input energy (AKA above the bound state energy) because this code does not converge to a state
    ...
    ///   - left_energy_scalar: Scalar for lower bound energy when checking log derivative
    ///   - right_energy_scalar: Scalar for upper bound energy when checking log derivative
    func schroedinger_photoionization_subroutine(z_value: Double, trial_energy: Double, number_blocks: Double, l_number: Double, number_points: Double, initial_delta_x: Double, mesh_scalar: Double, principal_quant_number: Double, input_pot_list: [Double], thresh_criterion: Double, max_thresh_iterations: Int, left_energy_scalar: Double, right_energy_scalar: Double, r_list: [Double]) {
        
//        print("Started Schrodinger Subroutine")
        ...
        // initializes  and calculates mesh, potential and KE list
        schroed_mesh_potential_init_inst.initialize_mesh_pot_KE_Photo(number_blocks: number_blocks, x_to_r_scalar: mesh_scalar, delta_x_initial: initial_delta_x, input_pot_list: input_pot_list, l_quant_number: l_number, input_energy: ncross_energy_approx, number_points: number_points, interpolated_r_mesh: r_list)
        ...
        schroed_mesh_potential_init_inst.KE_term_list.removeAll()
        // re-calculates KE list, KE index tuple and outward index tuple
        schroed_mesh_potential_init_inst.initialize_KE_term_list(scaled_pot_list: input_pot_list, scaled_r_list: r_list, l_quant_number: l_number, input_energy: ncross_energy_approx, number_blocks: number_blocks, number_points: number_points)
        //print(schroed_mesh_potential_init_inst.KE_term_list.count)
        // gets values from mesh_potential_init instance
        initial_KE_list = schroed_mesh_potential_init_inst.KE_term_list
        KE_index_tuple = schroed_mesh_potential_init_inst.KE_cross_index_tuple
...
//        print("initial photo outward")
        self.outward_integration_Photo(initial_r_list: r_list, initial_KE_list: initial_KE_list, number_blocks: number_blocks, l_number: l_number, trial_psi0_guess: trial_psi0_guess, number_points: number_points, KE_index_tuple: KE_index_tuple, outward_index_tuple: outward_index_tuple)
...
//        // calculates wavefunction value at outer radius with given energy
        func outer_radius_wf_value(energy_input: Double) -> Double {
//            print("called photoion outer radius func")
            // re-calculates KE list, KE index tuple and outward index tuple
            schroed_mesh_potential_init_inst.initialize_KE_term_list(scaled_pot_list: input_pot_list, scaled_r_list: r_list, l_quant_number: l_number, input_energy: ncross_energy_approx, number_blocks: number_blocks, number_points: number_points)
            ...
            // calculates wavefunction values from 0 to outer radius block
            self.outward_integration_Photo(initial_r_list: initial_r_list, initial_KE_list: initial_KE_list, number_blocks: number_blocks, l_number: l_number, trial_psi0_guess: trial_psi0_guess, number_points: number_points, KE_index_tuple: KE_index_tuple, outward_index_tuple: outward_index_tuple)
...
//
//        }
//        print("midpoint energy = \(mid_point_energy)")
...
//        let energy_thresh_criterion: Double = thresh_criterion
//        var calculated_energy_thresh_criterion: Double = 1.0
//        var final_energy: Double = trial_energy
//        var criterion_counter: Int = 0
...
//
//            // re-calculates KE list, KE index tuple and outward index tuple
//            schroed_mesh_potential_init_inst.initialize_KE_term_list(scaled_pot_list: input_pot_list, scaled_r_list: r_list, l_quant_number: l_number, input_energy: final_energy, number_blocks: number_blocks, number_points: number_points)
//
//            // gets values from mesh_potential_init instance
//            initial_KE_list = schroed_mesh_potential_init_inst.KE_term_list
//            KE_index_tuple = schroed_mesh_potential_init_inst.KE_cross_index_tuple
...
//
//
//            print(initial_r_list.max(), r_list.max())
//            // calculates wavefunction values from 0 to outer radius block
//            self.outward_integration_Photo(initial_r_list: r_list, initial_KE_list: initial_KE_list, number_blocks: number_blocks, l_number: l_number, trial_psi0_guess: trial_psi0_guess, number_points: number_points, KE_index_tuple: KE_index_tuple, outward_index_tuple: outward_index_tuple)
//
//            // calculates wavefunction values from matching radius to outer radius mesh point
...
//        }
        ...
        /// normalize wavefunctions
//        print(schroed_wavefunction_values_inst.outward_Pwavefunction_tuple_array[0].wavefunction_list.count)
        var wfVals: [Double] = []
        if(schroed_wavefunction_values_inst.outward_Pwavefunction_tuple_array[0].wavefunction_list.count > 60000){
            
        
        for i in stride(from: 20000, through: 60000, by: 1){
            wfVals.append(schroed_wavefunction_values_inst.outward_Pwavefunction_tuple_array[0].wavefunction_list[i].wavefunction_value)
        }
        }
    else{
        wfVals.append(1.0)
        }
    // normalization constants for inward and outward integration values
        let outward_normalization_constant: Double = pow(Double.pi,-0.5)*pow(trial_energy,-0.25)/(wfVals.max() ?? 1.0)
//        let inward_normalization_constant: Double = schroed_wavefunction_values_inst.wf_normaliztion_constant/schroed_wavefunction_values_inst.inward_wf_value
//
//        // For loop that multiplies each outward integration value by outward normaliztion factor and adds it to norm_Pwavefunction_tuple_array from origin to matching radius for each block
        
//        print(schroed_wavefunction_values_inst.outward_Pwavefunction_tuple_array.count)
//        print(schroed_wavefunction_values_inst.outward_Pwavefunction_tuple_array[0].wavefunction_list.count)
        var temp_wavefunction_tuple_array: [(r_value: Double, wavefunction_value: Double)] = []
        for j in stride(from: 0, to: schroed_wavefunction_values_inst.outward_Pwavefunction_tuple_array[0].wavefunction_list.count, by: 1){
...
//        for i in stride(from: 0, to: Int(number_blocks - 1), by: 1){
//
//            // Tuple array for r and wave function values for current block
//            var temp_wavefunction_tuple_array: [(r_value: Double, wavefunction_value: Double)] = []
//
...
//
//            // Last block
//            case Int(number_blocks - 1):
//
//                for j in stride(from: 0, to: Int(number_points)-1, by: 1) {
//
//                    temp_wavefunction_tuple_array.append((r_value: schroed_wavefunction_values_inst.outward_Pwavefunction_tuple_array[i].wavefunction_list[j].r_value, wavefunction_value: schroed_wavefunction_values_inst.outward_Pwavefunction_tuple_array[i].wavefunction_list[j].wavefunction_value/**outward_normalization_constant*/))
//
//                }
